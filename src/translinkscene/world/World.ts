...
export class World {
    ...
    private initScene(): void {
        ...
        this.scene.background = new THREE.Color(isDark ? 0x0c0e12 : 0xf5f1e8);
    ...
    private animate = (): void => {
        ...
        if (this.scene) {
            ...
            if (currentBgColor && currentBgColor.getHex() !== expectedColor) {
                currentBgColor.setHex(expectedColor);
                this.rootEl.dataset.theme = isCurrentlyDark ? 'dark' : 'light';
                this.cameraSettledFrames = 0; // Force update to paint the background
            }
        }
    ...
    private updateS4_1bRoadParticles(progress: number, delta: number): void {
        ...
        if (this.terrainSystem) {
            this.terrainSystem.setVisible(true);
            // Fade terrain smoothly near the scroll edges (0.0 to 0.05, and 0.95 to 1.0)
            const FADE_IN_END = 0.05;
            const FADE_OUT_START = 0.95;
            let opacity = 1.0;
            if (progress < FADE_IN_END) {
                ...
                opacity = (1.0 - progress) / (1.0 - FADE_OUT_START);
            }
            this.terrainSystem.setOpacity(opacity);
        }
    }
