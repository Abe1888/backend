...
export class TranslinkEasterEggFriend {
    ...
    private stopSpeechBtn: HTMLButtonElement | null = null;
    ...
    private _speechStoppedByUser = false;

    // Custom event handlers for mobile overlap fade
    private handlePopupOpen: ((e: Event) => void) | null = null;
    private handlePopupClose: ((e: Event) => void) | null = null;
    ...
    mount(parent: HTMLElement = document.body): void {
        ...
        Object.assign(this.buttonSlot.style, {
            position: 'absolute',
            left: isAr ? '-105px' : '95px', // Mirrored closer next to pointing arm to look naturally held
            ...
            opacity: '0',
            transform: isAr
                ? 'scale(0) translate3d(100px, 50px, -50px) rotate(30deg)' // Tucked behind the assistant initially (mirrored)
        ...
        // Emblem (red Translink logo) click/tap triggers voice session toggle
        ...
        // Create premium Stop Talking button
        ...
        // On mobile, hide the robot when a service/telemetry popup is opened to prevent overlap or accidental activation
        ...
        // Setup global listener to unlock browser audio restrictions upon first user click/scroll/keypress
        const resumeAudio = () => {
            ...
            // Remove listeners immediately once audio is unlocked
    ...
    returnHome(): void {
        ...
        const popupIsToRight = (this.facing > 0);

        // Smoothly zoom out the robot from its greeting closeup/zoom back to normal size
        tuckTl.to(this.creatureEl, {
    ...
    private _handleHoverStart(): void {
        ...
        // Smoothly ease body posture to frontal upright state, preserving greeting zoom if active
        if (this.creatureEl) {
            const isAr = TranslinkLanguageController.getInstance().getLanguage() === 'ar';
            const currentZoom = this.state === State.PRESENTING ? 2.4 : 1.0;
            gsap.to(this.creatureEl, {
                rotationX: 0,
                rotationY: 0,
                rotationZ: 0,
    ...
    private _playWelcomeSequence(): void {
        ...
        if (isMobile) {
            ...
            // Center the zoomed-in robot perfectly in the viewport for high premium impact (no notepad offset)
            ...
            welcomeTl.to(this.mover, {
                ...
                onComplete: () => {
                    ...
                    // Zoom/scale up the robot to 2.4x for a gorgeous premium close-up!
                    gsap.to(this.creatureEl, {
                        rotationX: 0,
                        rotationY: 0,
                        rotationZ: 0,
    ...
    private _initScrollTracking(): void {
        this.handleScroll = () => {
            ...
            // Debounced scroll stop detection (250ms of no scrolling)
            this.scrollTimer = setTimeout(() => {
                if (window.innerWidth <= 768) return; // Ignore scroll auto-triggering on mobile completely

                const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
                const scrollPercent = maxScroll > 0 ? (window.scrollY / maxScroll) * 100 : 0;

                // Map scrollPercent to deterministic section IDs based on translink_web_data.md
                let sectionId = '';
                ...
                // If stopped on a section S2 to S10, automatically present the popup
                if (sectionId !== 's1') {
                ...
                // Schedule wandering cycle after 10 seconds of no scrolling / absolute inactivity
                if (this.scrollIdleTimer) {
                    clearTimeout(this.scrollIdleTimer);
                }
    ...
    presentPopupForSection(sectionId: string): void {
        const isMobile = window.innerWidth <= 768;
        if (isMobile) {
            return; // In mobile view, the Robot must not trigger the TranslinkLiveFeedPopup card.
        }
        ...
        // Interrupt any active flight/tuck animations cleanly
        this._killFlight();
        this._releaseButton();

        this.currentSectionId = sectionId;

        // Create the popup on the fly
        ...
        this.activePopupEl = popupEl;

        if (isMobile) {
            this.state = State.PRESENTING;
            this._updateStateClasses('idle', 'confirming');

            // Close button click returns companion home
            ...
            // Animate popup presentation directly in center
            gsap.to(popupEl, {
                opacity: 1,
                scale: 1,
                duration: 0.5,
        ...
        // Desktop path — guard all required DOM references
        if (!this.mover || !this.shell || !this.buttonSlot || !this.floater || !this.creatureEl)
            return;
        ...
        // Customize popup styling for the mascot carry slot so it is relative and floats beautifully
        ...
        // Add to buttonSlot
        this.buttonSlot.innerHTML = '';
        this.buttonSlot.appendChild(popupEl);
        ...
        // Determine current position of the robot in viewport coordinates
        const robotRect = this.creatureEl!.getBoundingClientRect();
        const robotViewportX = robotRect.left + robotRect.width / 2;
        const activeSide = robotViewportX < window.innerWidth / 2 ? 'left' : 'right';

        // Position buttonSlot and flip scaleX based on the activeSide
        if (activeSide === 'left') {
            this.buttonSlot.style.left = '95px';
        } else {
            this.buttonSlot.style.left = '-345px';
        }

        // Keep popup slot hidden and tucked behind assistant initially during flight
        gsap.set(this.buttonSlot, {
            opacity: 0,
            scale: 0,
        ...
        // Determine destination coordinates for the mascot presentation
        const robotWidth = 90;
        const moverX = gsap.getProperty(this.mover, 'x') as number;
        const moverY = gsap.getProperty(this.mover, 'y') as number;
        const shellRect = this.shell!.getBoundingClientRect();
        ...
        this.flightTl = gsap.timeline({
            ...
            onComplete: () => {
                ...
                this.facing = activeSide === 'left' ? 1 : -1;
                this.currentFacing = activeSide === 'left' ? 1 : -1;

                // Animate popup presentation
                this._animateButtonPresentation();
            },
        });

        // Cinematic curved spline flight to destination
        const dx = targetX - startX;
        const dy = targetY - startY;
        this.flightTl.to(this.mover, {
            duration: 2.0,
    ...
    private _startWanderingCycle(): void {
        if (window.innerWidth <= 768) return; // Disable wandering on mobile
    ...
    private async _toggleVoiceSession(): Promise<void> {
        ...
        if (vm.isConnected()) {
            ...
            this._speechStoppedByUser = true; // User stopped speech manually!
            vm.disconnect();
            return;
        }

        console.log('[Companion] Emblem clicked — initiating chat voice session.');
        this._speechStoppedByUser = false; // Reset silent mode on manual user initiation
        this._setWelcomedThisSession();
        if (this.brain) {
    ...
    private _handleVoiceStateChange(voiceState: VoiceState): void {
        ...
        // Dynamic Stop Talking control visibility synchronization
        if (this.stopSpeechBtn) {
            if (voiceState !== 'idle') {
                gsap.to(this.stopSpeechBtn, {
                    display: 'flex',
                    opacity: 1,
                    scale: 1,
                    y: 0,
                ...
                gsap.to(this.stopSpeechBtn, {
                    ...
                    onComplete: () => {
                        if (this.stopSpeechBtn) {
                            this.stopSpeechBtn.style.display = 'none';
                        }
                    }
                });
            }
